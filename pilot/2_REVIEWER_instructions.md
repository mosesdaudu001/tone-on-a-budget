...
- **Pre-registered success line (decided in advance, do not move it):** **AUROC ≥ 0.70** **or** ✓/✗ accuracy ≥ 75% on the known-correct native anchors = "preliminary human evidence the metric tracks perceived tone." Anything lower is reported honestly as an inconclusive pilot. (AUROC, not κ, is the headline: it needs no score cutoff, so it isn't depressed by the native-anchor-median binarization that makes κ read conservative.)
